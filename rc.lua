...
-- {{{ Tags
tags = {
   names = { " WEB ", "DEV", " TERMINAL ", " FILES ", "REMOTE", " OTHER " },
...
lain.widgets.calendar:attach(calendarwidget, { fg = "#FFFFFF", position = "bottom_right", font = "UbuntuMono" })

-- Mail IMAP check
-- commented because it needs to be set before use
--[[
mail_widget = lain.widgets.imap({
    timeout  = 180,
    server   = "mail.cmsrv.org",
    mail     = "oompaloompa@cmsrv.org",
    password = "ExVwGFF7XduyvaT3Cgi2f45W6L9iIe6d4z6vemQrMc0grCKfRERa5UzUtxrh",
    is_plain = true,
    settings = function()
...
mail_icon:set_image(beautiful.mail)
...
bat_icon:set_image(beautiful.battery)
...
for s = 1, screen.count() do
    ...
    --bottom_right_layout:add(mail_icon)
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey,       }, "F3"    , function() awful.util.spawn("pcmanfm-qt", false)                    end)
